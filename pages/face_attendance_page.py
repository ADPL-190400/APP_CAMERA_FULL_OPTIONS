...
class FaceCaptureWorker(QThread):
    ...
    def _run_detect(self, frame: np.ndarray) -> None:
        ...
        # Nhiều mặt trong khung -> chỉ xét mặt LỚN NHẤT (gần cam nhất) -
        # kiosk giả định 1 người thao tác tại 1 thời điểm.
        face = max(good_faces, key=lambda f: (f.bbox[2] - f.bbox[0]) * (f.bbox[3] - f.bbox[1]))
...
class EnrollWorker(QThread):
    ...
    def __init__(self, employee_data: dict, embeddings: list, avatar_frame, avatar_bbox, parent=None):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.finished_ok.emit(False, f"Lỗi lưu thông tin lên server: {exc}")
            return

        self._save_avatar()
        self.finished_ok.emit(True, "Đã lưu thông tin nhân viên thành công.")
...
_MAIN_HINT_DEFAULT = "Đứng thẳng trước camera, giữ khuôn mặt trong khung hình"
...
class FaceAttendanceWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Face App - Attendance")
    ...
    def _build_ui(self) -> None:
        ...
        lbl_title = QtWidgets.QLabel("🪪  FACE ATTENDANCE")
        lbl_title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        lbl_title.setStyleSheet("color: #3a5070; font-size: 13px; font-weight: 700; letter-spacing: 3px;")
        ...
        self.lbl_status = QtWidgets.QLabel("Đang tìm khuôn mặt...")
        self.lbl_status.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.lbl_status.setStyleSheet("color: #e0e6f0; font-size: 26px; font-weight: 700;")
        root.addWidget(self.lbl_status)

        self.lbl_hint = QtWidgets.QLabel(_MAIN_HINT_DEFAULT)
        ...
        self.btn_action = QtWidgets.QPushButton("➕  Đăng ký")
    ...
    def _on_face_state(self, state: Optional[dict]) -> None:
        ...
        if state is None:
            self.lbl_status.setText("Đang tìm khuôn mặt...")
            self.lbl_hint.setText(_MAIN_HINT_DEFAULT)
            self.btn_action.setVisible(False)
        ...
        self.lbl_hint.setText("Lại gần camera hơn một chút" if face_ratio < _MAIN_MIN_FACE_RATIO else _MAIN_HINT_DEFAULT)

        if employee is not None:
            name = f"{employee.get('last_name') or ''} {employee.get('first_name') or ''}".strip()
            self.lbl_status.setText(f"Xin chào, {name or '?'}")
            self.btn_action.setText("✎  Sửa thông tin")
            self.scan_view.set_state(RingState.LOCKED_KNOWN if stable else RingState.SEARCHING)
        else:
            self.lbl_status.setText("Chưa nhận diện được - người mới")
            self.btn_action.setText("➕  Đăng ký")
    ...
    def _on_action_clicked(self) -> None:
        ...
        self._start_enroll(employee_data, wizard.embeddings, wizard.avatar_frame, wizard.avatar_bbox)

    def _start_enroll(self, employee_data: dict, embeddings: list, avatar_frame, avatar_bbox) -> None:
        self.btn_action.setEnabled(False)
        self.lbl_status.setText("⏳ Đang lưu thông tin...")
        self._enroll_worker = EnrollWorker(employee_data, embeddings, avatar_frame, avatar_bbox, parent=self)
